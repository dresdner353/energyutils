...
* For each hour, the first step performed is to steal export if export exists and the simulated battery has capacity to take on more charge. Then the import for the same is checked and offset from the battery storage. So this gives a best possible outcome in favour of the battery per hour by acting as if all the export happened before all the import took place. 
