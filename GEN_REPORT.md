...
* --tariffs <file.json>
Specifies one or more tariff plans defined in a JSON file. See [Tariff Plan File Example](./sample_tariffs_plan.json) for an example of this file. Essentially the file is a JSON list of objects, one per plan. Each plan defines the start and end date in YYYY-MM-DD format. Then additional fields are specified for annual standing charge, fit rate and a list of tariffs for the plan. Each tariff defines the start/end hours, week days (1-7 if applicable) and the rate per kWh.
* --reports [{year,month,week,day,hour,tariff,weekday,24h} ...]  
